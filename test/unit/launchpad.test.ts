...
describe("Launchpad unit tests", async function () {
  ...
  describe("Distribute Funds", async () => {
    ...
    it("Checking Treasury, Profit and Loyalty address", async () => {
      ...
      const signer0BalanceBefore = BigInt((await erc20.balanceOf(signers[0].address)).toString())
      ...
      assert.equal(
        (loyalty1 + signer0BalanceBefore + loyalty2).toString(),
        (await erc20.balanceOf(signers[0].address)).toString()
      )
      //   assert.equal(
