  ...
  ReentrancyGuardUpgradeable
{
  ...
  bool public IS_REQUIRED_BOOST_TO_UPGARDE = true;
  ...
  function initialize(
    ...
    __ERC721_init("WINERY DAO", "NFT DAO");
  ...
  function decreaseRB(
    ...
    uint256 minDecreaseLevel,
    address user
  ) external onlyRole(RB_SETTER_ROLE) returns (uint256 decreaseAmount) {
    require(decreasePercent <= 1e12, "Wrong decrease percent");
    uint256[] memory finalRB = new uint256[](tokensId.length);
    decreaseAmount = 0;
    for (uint256 i = 0; i < tokensId.length; i++) {
      ...
      uint256 currentRB = _tokens[tokensId[i]].robiBoost;
  ...
  function decreaseRBView(
    ...
    uint256 decreasePercent,
    uint256 minDecreaseLevel
  ) external view returns (uint256 decreaseAmount) {
    require(decreasePercent <= 1e12, "Wrong decrease percent");
    decreaseAmount = 0;
    for (uint256 i = 0; i < tokensId.length; i++) {
      if (_tokens[tokensId[i]].level <= minDecreaseLevel) continue;
  ...
  function getRbBalanceByDays(address user, uint256 dayCount)
    public
    view
  ...
  function levelUp(uint256[] calldata tokenId) public nonReentrant {
    ...
    if (!IS_REQUIRED_BOOST_TO_UPGARDE) {
      cumulatedRb = neededRb;
    }
  ...
  function sendRBToToken(uint256[] calldata tokenId, uint256[] calldata amount)
    public
    nonReentrant
  {
  ...
  function increaseRBToken(uint256[] calldata tokenId, uint256[] calldata amount)
    public
    nonReentrant
    onlyRole(RB_SETTER_ROLE)
  {
    _sendRBToToken(tokenId, amount);
  }

  function exchangeRB(uint256 amount, address userAddress)
    public
    nonReentrant
    onlyRole(RB_SETTER_ROLE)
    returns (bool)
  {
    uint256 calcAmount = amount;
  ...
  function decreaseRobiBoost(
    ...
    require(
      _robiBoost[user][day] >= amount && _robiBoostTotalAmounts[day] >= amount,
      "Wrong amount"
    );
    _robiBoost[user][day] -= amount;
    _robiBoostTotalAmounts[day] -= amount;
