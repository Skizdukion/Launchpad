...
error NFTDao__WrongPercentageValue();

contract NFTDao is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function mint(address to, uint256 level, uint256 robiBoost) public onlyRole(TOKEN_MINTER_ROLE) nonReentrant {
    ...
    function sendRBToToken(uint[] calldata tokenId, uint[] calldata amount) public nonReentrant {
     ...
     function increaseRBToken(uint[] calldata tokenId, uint[] calldata amount) public nonReentrant onlyRole(RB_SETTER_ROLE) {
        _sendRBToToken(tokenId, amount);
    }
